...
$(document).ready(function() {
	...
	// Map: render map
	if(!devMode){
		L.mapbox.accessToken = 'pk.eyJ1Ijoic3Rpam4iLCJhIjoiZDloUDlGQSJ9.Xl45H6NiRTTxFwxosQU-cQ';
		var map = L.mapbox.map('map', mapId, {scrollWheelZoom: false, zoomControl: false}).setView([lat, lon], zoom);
			map.dragging.disable();
			map.touchZoom.disable();
			map.doubleClickZoom.disable();
			map.scrollWheelZoom.disable();
		var mapBottom = L.mapbox.map('map-bottom', mapId, {scrollWheelZoom: false, zoomControl: false}).setView([lat, lon], zoomBottom);	
